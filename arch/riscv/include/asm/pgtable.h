...
{
        uint32_t page_addr = ((uint32_t)pte_val(pteval) >> 10) << 12;
	...
	// map immediately for kernel stuff
	if ((addr & 0xF0000000) >= 0xA0000000) {
		// VMALLOC
		vexriscv_mmu_map((addr >> 12) & 0xFFFFF, 0xF0000000 | ((page_addr>>12)&0xFFFFF), page_location++ % PAGE_COUNT); // kernel only
	}
}
