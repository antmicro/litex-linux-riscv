...
static void cs(struct spi_nor *nor, u8 new_val)
{
	...
	u8 curr_val = litex_read_reg(spi->base + SPIFLASH_BITBANG_OFFSET);
	u8 set_val = new_val == CS_ENABLE ?
		curr_val | new_val : curr_val & new_val;

	litex_write_reg(set_val, spi->base + SPIFLASH_BITBANG_OFFSET);
}
...
static u8 miso_read(struct spi_nor *nor)
{
	...
	return (u8)(litex_read_reg(spi->base + SPIFLASH_MISO_OFFSET) & 0x1);
}
...
{
	struct spi *spi = nor->priv;
	u32 curr_val = litex_read_reg(spi->base + SPIFLASH_BITBANG_OFFSET);
	u32 set_val = new_val == MISO_MODE ?
		curr_val | new_val : curr_val & new_val;
...
static void initial_config(struct spi_nor *nor)
{
	...
	litex_write_reg(SPIFLASH_ENABLE,
...
static int litex_spi_flash_remove(struct platform_device *pdev)
{
	...
	res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	spi->base = devm_ioremap_resource(&pdev->dev, res);
	litex_write_reg(SPIFLASH_DISABLE,
		spi->base + SPIFLASH_BITBANG_EN_OFFSET);
