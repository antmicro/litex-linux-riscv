...
{
	iowrite32(val, port->membase + offset);
}
...
DBG();
	...
	DBGMSG("port pointer is %X", port);

	/* Has the device been initialized yet? */
	
	if (!port->mapbase) {
	DBGMSG("We do not have mapbase!");
	...
	if (!port->membase) {
		if (litex_uart_request_port(port)) {
		 DBGMSG("We have a problem!");
			return -ENODEV;
			}
	}
 DBGMSG("We are going to parse options, located at 0x%08X!", options);
	if (options)
		uart_parse_options(options, &baud, &parity, &bits, &flow);

           DBGMSG("Parsed, baud=%d, port is %X, console is %X", baud, port, co);
...
DBG();
	...
	DBGMSG("start the uart assing, base = %X", base);
...
//		return -ENXIO;
            ...
            DBGMSG("We are going to make it :)");
...
DBG();
	...
	if (ret) {
	DBGMSG("We could not register uart!");
		goto err_uart;
		}

	ret = platform_driver_register(&litex_uart_platform_driver);
	if (ret) {
	        DBGMSG("We could not register the driver!");
		goto err_plat;
		}
